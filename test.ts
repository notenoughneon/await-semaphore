...
describe('util', function () {
    describe('semaphore', function () {
        ...
        it('limits concurrency and run tasks with priority', async function () {
            var s = new Semaphore(1);
            var order: number[] = [];
            var task = async (i: number) => {
                var release = await s.acquire(i);
                order.push(i);
                release();
            };
            await Promise.all([3, 1, 2, 1].map(i => task(i)));
